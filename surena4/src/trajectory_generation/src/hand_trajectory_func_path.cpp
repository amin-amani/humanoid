        ...
        // looking at horizon
            if(scenario=="d"){
                 ...
                 r_target<<.3,
                          0.1,
                         0.25;
                 R_target=hand_funcs.rot(2,-90*M_PI/180,3)*hand_funcs.rot(1,90*M_PI/180,3)*hand_funcs.rot(3,90*M_PI/180,3);
                 r_middle<<.4,
                          -.2,
                         -0.1;
            }
...
if(time<t(1)){
    ...
    if(scenario=="g" && time<8){q_ra(2)=qr2_end+15*M_PI/180*sin((time-t(2))/2*(2*M_PI));}
    else if(scenario=="f" && time<8){q_ra(3)=qr3_end+5*M_PI/180*sin((time-t(2))/2*(2*M_PI));}
    else{ROS_INFO("reached!");break;}
}
