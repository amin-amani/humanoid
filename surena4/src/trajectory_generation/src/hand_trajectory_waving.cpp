...
//                -0.2,
//                0.20;
//R_target=hand_funcs.rot(2,-170*M_PI/180,3)*hand_funcs.rot(3,90*M_PI/180,3);
//R_target=hand_funcs.rot(1,-90*M_PI/180,3)*hand_funcs.rot(3,-90*M_PI/180,3);
R_target=hand_funcs.rot(2,-160*M_PI/180,3)*hand_funcs.rot(3,90*M_PI/180,3);
//    R_target=hand_funcs.rot(2,-90*M_PI/180,3)*hand_funcs.rot(1,-90*M_PI/180,3);
//    r_target<<0,
//            -.550,
//            0;
...
ROS_INFO("\nsai_target=%f\tphi_target=%f\ttheta_target=%f",sai_target,phi_target,theta_target);
...
int count = 0;
double waving_time=0;
double q_waving=0;
double wavingcount=0;
    while (ros::ok())
    {
//        for (int k = 0; k < 7; ++k) {
...
//while (1){
        ...
        else if ((d>d_des)  || (abs(theta-theta_target)>.5)   || (abs(sai-sai_target)>.8)  || (abs(phi-phi_target)>.5)  )
{

        right_hand hand(q_ra,r_target,R_target,i,d0);
//hand.Right_palm_orientation_power=0;
...
//         q[17]=-10*M_PI/180;
    ...
    //   ROS_INFO("T=%f upper:%f %f lower:%f %f qdot=%f q=%f %f",hand.T,(hand.toRad(10)+q_ra(1))/hand.T,hand.qdot_max,(hand.toRad(-90)+q_ra(1))/hand.T,-hand.qdot_max,hand.qdot(2),180/M_PI*q[16],180/M_PI*q_ra(1));
        ...
        else if(wavingcount<2){

            q_ra(2)=q_waving+M_PI/10*(-1+sin(waving_time*3+M_PI/2));

           // ROS_INFO("q_ra(2)=%f",q_ra(2));
            waving_time+=hand0.T;

            wavingcount=2*waving_time*3/(2*M_PI);
            qr1.append(q_ra(0));
            qr2.append(q_ra(1));
            qr3.append(q_ra(2));
            qr4.append(q_ra(3));
...
//       ROS_INFO("phi=%f(%f)\ttheta=%f(%f)\tsai=%f(%f)\t",phi,phi_target,theta,theta_target,sai,sai_target);
//       ROS_INFO("x=%f\ty=%f\tz=%f\t",hand.r_right_palm(0),hand.r_right_palm(1),hand.r_right_palm(2));
//       ROS_INFO("w_x=%f,w_y=%f,w_z=%f",hand.w_right_palm(0),hand.w_right_palm(1),hand.w_right_palm(2));
        q[15]=qr1[count];
        q[16]=qr2[count];
        q[17]=qr3[count];
        q[18]=qr4[count];
      ...
      //  SendGazebo(q);
       ...
       q_motor[0]=-int(10*(qr1[count]-q0(0))*360/M_PI);
       q_motor[1]=int(10*(qr2[count]-q0(1))*360/M_PI);
       q_motor[2]=-int(7*(qr3[count]-q0(2))*300/M_PI);
       q_motor[3]=int(7*(qr4[count]-q0(3))*300/M_PI);
//ROS_INFO("%f %d",qr4[count]*180/M_PI,q_motor[3]);
